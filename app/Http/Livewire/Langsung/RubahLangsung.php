...
class RubahLangsung extends Component
{
    use WithFileUploads;
    ...
    public function LanjutSurat($id)
    {
        try {
            $resSurvey = [
                'permohonan_id' => 4,
    ...
    public function KirimPending($id)
    {
        $dataNotifikasi = [
            'notifikasi_kirim' => "Y",
            'permohonan_id' => 2
        ...
        Histori::where('no_register', $id)->where('permohonan_id', 2)->delete();
        $this->success();
    }

    public function render()
    {
        $dataList = User::with(['kategori', 'persyaratan', 'dokumen', 'ketua', 'sekretaris', 'bendahara', 'pendiri', 'pembina', 'penasihat', 'kategori'])->where('permohonan_id', 6)->get();
        $dataKelurahan = Kelurahan::All();
        $dataKecamatan = Kecamatan::All();
        $dataKota = Kota::All();
        return view('livewire.langsung.rubah-langsung', compact('dataList','dataKelurahan', 'dataKecamatan', 'dataKota'));
    }
}
