...
class ListVerifikasi extends Component
{
    ...
    public function ShowPersyaratan($id)
    {
        ...
        $this->kepengurusanTemp = $resPersyaratan->persyaratan->kepengurusan_id_ormas;
        $PengurusV = Kepengurusan::all()->where('id', $this->kepengurusanTemp)->first();
        $this->kepengurusan = $PengurusV->kepengurusan;


        $this->bidangTemp = $resPersyaratan->persyaratan->bidang_id_ormas;
        $this->subbidangTemp = $resPersyaratan->persyaratan->subbidang_id_ormas;
        $BidangV = Bidang::all()->where('id', $this->bidangTemp)->first();
        $SubbidangV = Subbidang::all()->where('id', $this->subbidangTemp)->first();
        $this->bidang = $BidangV->bidang;
        $this->subbidang = $SubbidangV->subbidang;
    ...
    public function store_persyaratan()
    {
        if ($this->st_verifikasiSyarat == 0) {
            ...
            try {
                ...
                $dt_status_syarat = ['status_persyaratan' => "Y", 'feedback_persyaratan'=> 'N'];
                User::where('no_register', $this->noreg)->update($dt_status_syarat);
    ...
    public function store_pengurus()
    {
        ...
        if (
            ...
            $dt_status_pengurus = ['status_pengurus' => "N", 'feedback_pengurus' => null];
    ...
    public function store_dokumen()
    {
        ...
        if (
            ...
            $dt_status_dokumen = ['status_dokumen' => "N",  'feedback_dokumen' => null];
    ...
    public function LanjutSurvey($id)
    {
        try {
            $resSurvey = [
                'permohonan_id' => 3,
    ...
    {
        $dataNotifikasi = ['notifikasi_kirim' => "Y"];
        User::where('no_register', $id)->update($dataNotifikasi);
        $this->success();
    }

    public function render()
    {
        $dataList = User::with(['kategori', 'persyaratan', 'dokumen', 'ketua', 'sekretaris', 'bendahara', 'pendiri', 'pembina', 'penasihat', 'kategori'])->where('permohonan_id', 2)->get();
        $dataKelurahan = Kelurahan::All();
        $dataKecamatan = Kecamatan::All();
        $dataKota = Kota::All();
        return view('livewire.verifikasi.list-verifikasi', compact('dataList', 'dataKelurahan', 'dataKecamatan', 'dataKota'));
    }
}
