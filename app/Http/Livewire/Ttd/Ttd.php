...
namespace App\Http\Livewire\Ttd;
...
class Ttd extends Component
{
    ...
    public function ShowPersyaratan($id)
    {
        ...
        // $this->bidang = $resPersyaratan->persyaratan->bidang_id_ormas;
        // $this->subbidang = $resPersyaratan->persyaratan->subbidang_id_ormas;
    ...
    public function acc($id)
    {
        try{
            ...
            SuratKeberadaan::where(['no_register' => $id, 'status' => 'A'])->update($resSurat);
            ...
            $dataForPDF = User::with(['kategori', 'persyaratan', 'dokumen', 'ketua', 'sekretaris', 'bendahara', 'pendiri', 'pembina', 'penasihat', 'srtkeberadaan'])->where('no_register', $id)->first();
            $Ambilpejabat = SuratKeberadaan::where('id_ttd', Auth::user()->id)->first();
            $pejabatTtd = User::where('id', $Ambilpejabat->id_ttd)->first();

            $Nomor = SuratKeberadaan::where('no_register', $id)->count();
            ...
            $tglsurat = Carbon::parse($dataForPDF->srtkeberadaan[0]->tanggal_surat)->format('Ymd');
            ...
            $documentPDF = PDF::loadView('backend.pdf.pdf-ormas', $contentPDF)->setPaper('A4', 'portrait');
            // $filename = 'surat_pdf/'.Str::random(8).'.pdf';
            $filename = 'surat_pdf/'. $id . '_' .$tglsurat . '_' . $Nomor .'.pdf';
            Storage::put($filename, $documentPDF->output());

            $resSuratFile = ['file_surat'=> $filename];
            SuratKeberadaan::where(['no_register' => $id])->update($resSuratFile);
    ...
    public function render()
    {
        ...
        //     'pembina', 
        //     'penasihat',
        //     'srtkeberadaan'])->where('permohonan_id','>=', 5)->get();
        ...
        $dataUser = User::all();
        return view('livewire.ttd.ttd', compact('dataList','dataUser'));
    }
}
