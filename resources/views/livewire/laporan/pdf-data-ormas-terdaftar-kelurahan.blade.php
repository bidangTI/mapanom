                ...
                @foreach ($dataOrmas as $valOrmas)
                    @foreach ($kelurahan as $valkelurahan)
                        @if ($valkelurahan->id == $valOrmas->persyaratan->kelurahan)
                            <b>DATA ORMAS TERDAFTAR DI KELURAHAN {{ Str::upper($valkelurahan->nama_kelurahan) }} KECAMATAN {{ Str::upper($valkelurahan->datakecamatan->nama_kecamatan) }} <br>KOTA SURAKARTA</b>
                        @endif
                    @endforeach
                @endforeach
            </span>
