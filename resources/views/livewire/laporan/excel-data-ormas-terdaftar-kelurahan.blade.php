...
<table width="100%" style="margin-bottom: 20px">
                ...
                @foreach ($dataOrmas as $valOrmas)
                    @foreach ($kelurahan as $valkelurahan)
                        @if ($valkelurahan->id == $valOrmas->persyaratan->kelurahan)
                            <b>DATA ORMAS TERDAFTAR DI KELURAHAN {{ Str::upper($valkelurahan->nama_kelurahan) }} KECAMATAN {{ Str::upper($valkelurahan->datakecamatan->nama_kecamatan) }}</b>
                        @endif
                    @endforeach
                @endforeach
            </span>
    ...
    <tr>
        <td colspan="9">
            <span style="font-family: Arial, Helvetica, sans-serif; font-size: 12pt"><b>KOTA SURAKARTA</b></span>
        </td>
    </tr>
</table>
