                ...
                <div class="table-responsive">
                    <table id="zero_config" class="table table-striped table-bordered text-nowrap">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>No. Register<br>Nama Ormas/Parpol</th>
                                <th>Nama Pendaftar<br>Progres Pendaftaran</th>
                                <th>Alamat</th>
                                <th>email</th>
                                <th>Roles</th>
                                <th>Status User</th>
                        ...
                        <tbody>
                            ...
                            @forelse ($dataLoadlist as $Nomer => $valList)
                                <tr>
                                    <td>{{ $Nomer + 1 }}</td>
                                    <td>
                                        <b>{{ $valList->no_register }}</b><br>
                                        @if (!empty($valList->persyaratan))
                                            {{ $valList->persyaratan->nama_ormaspol }}
                                        @endif
                                        <br><span
                                            class="badge bg-light-success text-success font-medium">{{ $valList->kategori->kategori }}</span>
                                    </td>
                                    <td>
                                        {{ $valList->name }}<br>
                                        @if ($valList->permohonan_id == 6)
                                        <span
                                            class="badge bg-light-success text-success font-medium">Terdaftar</span>
                                        @else
                                        <span
                                        class="badge bg-light-warning text-warning font-medium">
                                        {{ $valList->permohonan->status }}</span>
                                        @endif
                                    </td>
                                    @if (empty($valList->alamat))
                                        <td>-</td>
                                    @else
                                        <td>{{ $valList->alamat }}<br>
                                            RT. {{ $valList->rt }}<br>
                                            RW. {{ $valList->rw }}
                                        </td>
                                    @endif
